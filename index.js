...
Bienvenid@ a 
*${mdata.subject}*
Por favor disfruta de estar a qui
-NO PERMITIMOS EL PORNO 
-Todos abusan de mi :c 
-Si tambien quieres 
Usa ${prefix}verify para comenzar y poder abusar del bot Ana.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `el compadre  @${num.split('@')[0]} lamentablemente se ha ido`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: '⏳ Esperame uwu ',
  success: '🎉 Con exito uwu',
  Public: '🛡 Modo privado activado',
  ferr: 'ay! ha ocurrido un error por favor comunica a mi creador :c',
  ...
  only: {
    ...
    ownerG: 'Este comando solo puede ser usado por mi amo, el es el unico que puede abusarme de esa y mas formas',
    ownerB: 'Este comando solo puede ser usado por el dueño del bot',
    admin: 'Debes de ser uno de los todo poderosos ultra facheros admins de este grupo!',
    Badmin: 'a un no me an dado admin :c se nota que no me quieren tal vez saldre ',
    daftarB: `Hola cosita hermosa presiosa *${prefix}verify* luego podras usarme bb uwu`
  }
}
...
if (getmemex <= memberlimit) {
  Lxa.sendMessage(from, `Los miembros de este grupo no cumplen con el minimo requerido *${memberlimit}*  para usar este bot`, text)
...
switch(is) {
	case 'bot':
buf = fs.readFileSync(`./temp/audio/bot10.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
        case '5cm':
buf = fs.readFileSync(`./temp/audio/5cm.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
        case 'te_amo':
buf = fs.readFileSync(`./temp/audio/Teamo.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
        ...
        case 'ctmr':
        case 'conchetumare':
buf = fs.readFileSync(`./temp/audio/ctm.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
               case 'reza':
buf = fs.readFileSync(`./temp/audio/reza.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
		case 'sexo':
buf = fs.readFileSync(`./temp/audio/sexo10.mp3`)
Lxa.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, ptt: true
})
break
...
case 'menu':
hasil = `        ────────────────
Hola, ese comado no es :c  *${pushname}* puedes usar este comando ${prefix}menu a si podras usarme todo lo que quieras 
        ────────────────`
reply(hasil)
        break
}
...
case '?':
  ...
  const Menu = {
text: `  ──── *comandos de bot Ana* ────
────────────────
Hola *${pushname}* puedes usar uno de mis comandos
...
pok = await getBuffer(nimek)
Lxa.sendMessage(from, pok, image, {
  quoted: mek, caption: `Sexi Ana-Bot`
})
...
method: 'get'
  ...
  pok = await getBuffer(nimek)
  Lxa.sendMessage(from, pok, image, {
quoted: mek, caption: `señorita-Ana`
  })
...
//-- temp
			case 'gets':
				...
				Lxa.sendMessage(from, result, sticker, selepbot)
				  ...
				  reply('lo lamento no encontre el paquete que buscas pero si quieres te doi el mio')
				}
				break
...
//clear all chat
				case 'clearall':
					...
					break
				case 'bc':
					if (!isOwner) return reply('lo siento, yo solo hago lo que mi amo dice c:')
					if (args.length < 1) return reply('dime que cosas quieres anunciar bb')
					anu = await Lxa.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Oppa! ya di el anuncio ¿me daras las gracias c:?')
					}
					break
...
case 'setppbot':
  ...
  reply('Oppa gracias, esta foto me queda mas bonita uwu?')
  break
...
//--Convert stiker to image
				case 'toimg':
					if (!isQuotedSticker) return reply('pudes responderme a una foto o gif para cumplir mi trabajo uwu?')
					if (!isRegister) return reply(mess.only.daftarB)
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Sex Ana'})
						fs.unlinkSync(ran)
					})
					break
...
case 'bot':
					if (args.length < 1) return reply(`mmm...  ${pushname} que pasu?`)
					teks = body.slice(5)
					try { 
...
case '.uwu':
  case 'register':
if (isRegister) return reply('oppa... tu ya puede usarme cuando quieras')
...
Lxa.deleteMessage(from, {
  ...
  reply('Oppa solo puedo borrar mensajes míos...')
}
break
...
break
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('lo siento :c ya lo as activado')
						welkom.push(from)
						fs.writeFileSync('./data/welkom.json', JSON.stringify(welkom))
						reply('Oppa! la bienvenida esta activa')
					} else if (Number(args[0]) === 0) {
  ...
  reply(`        ────────────────\nHola uwu  *${pushname}* !!!\nEse comando no esta en mi lista : *${prefix}${command}*\nverifica si lo a escrito bien en*${prefix}Menu* si lo as puesto por accidente ignorame c:\n        ────────────────`)
				}
